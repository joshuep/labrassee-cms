...
function buildMailto(iso) {
  const [y, m, d] = iso.split('-').map(Number)
  const dateObj = new Date(y, m - 1, d)
  const jourLong = JOURS_FR_LONG[dateObj.getDay()]
  const sujet = encodeURIComponent('Proposition Sur la scène - ' + iso)
  const corps = encodeURIComponent(
    'Salut !\n\n' +
    `Date visée : ${jourLong} ${d} ${MOIS_FR[m - 1]} ${y}\n\n` +
    'Nom artiste / groupe : \n' +
    'Genre : \n' +
                ...
                {m.jours.map((j, idx) => {
                  ...
                  return (
                    <Case
                      ...
                      onClick={isClickable ? () => { window.location.href = buildMailto(j.iso) } : undefined}
                    >
